...
class ImageController extends BaseController
{
    ...
    {
        $template = $this->getTemplate($template, $filename);
     ...
     * @return mixed
     */
    private function getTemplate($template, $filename)
    {
        $templates = settings('images');

        if(!$templates){
            return function (ImageCache $image) {
                return $image->fit(250, 250);
            };
        }

        if(array_key_exists($template, $templates)){
            $size = $templates[$template];
            return function (ImageCache $image) use ($size){
